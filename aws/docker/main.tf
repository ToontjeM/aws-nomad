...
data "lacework_agent_access_token" "docker" {
...
module "docker_iam_instance_profile" {
...
module "docker_server" {
  ...
  vpc_security_group_ids = [module.docker_security_group.id]
  iam_instance_profile   = module.docker_iam_instance_profile.instance_profile_id
  tags = {
...
resource "null_resource" "docker_server" {
  ...
  connection {
    type        = "ssh"
    host        = module.docker_server[count.index].public_ip
    user        = "ubuntu"
  ...
  provisioner "file" {
    content = templatefile("${path.module}/templates/consul-server.hcl", {
      server_count = var.docker_server_count
...
#  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo /tmp/lw-install.sh -U https://api.lacework.net ${data.lacework_agent_access_token.docker.token}",
